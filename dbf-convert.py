...
def run(dbfName):
    global conn
    table = dbf.VfpTable(dbfName)
    table.open()
    
    if args.verbose: print "--> " + dbfName + " <--"
    if args.table:
        for fldName in table.field_names:
        ...
        cr_stat = "CREATE TABLE `%s` (`%s`)" % (netto_name, '`,`'.join(table.fieldNames))
        ...
        for i1 in range(min(3,len(table))):
            rec = table[i1]
            for fldName in table.fieldNames:
